#!/usr/bin/env python3
"""
Command-line interface for the arXiv paper download tool.

This module handles the user interaction through the command line,
providing menus and options to search, download, and manage arXiv papers.
"""
...
from metadata import extract_and_save_metadata
from utils import (
...
def process_paper(xml_response):
    """
...
def search_paper(query, limit=5, auto_download=False):
    """
...
def fetch_metadata_for_imported_papers():
    """
...
def process_existing_directories():
    """
    ...
    for d in all_dirs:
        ...
        if is_valid_arxiv_id(dir_name):
            ...
            if os.path.exists(os.path.join(base_dir, 'metadata')) or os.path.exists(os.path.join(base_dir, 'pdf')):
    ...
    for paper_dir in paper_dirs:
        ...
        has_metadata = os.path.exists(os.path.join(paper_dir, 'metadata')) and len(os.listdir(os.path.join(paper_dir, 'metadata'))) > 0
        ...
        if has_metadata:
            # Extract title
            title_path = os.path.join(paper_dir, 'metadata', 'title.txt')
            if os.path.exists(title_path):
                with open(title_path, 'r', encoding='utf-8') as f:
                    title = f.read().strip()
            
            # Extract authors
            authors_path = os.path.join(paper_dir, 'metadata', 'authors.txt')
            if os.path.exists(authors_path):
                with open(authors_path, 'r', encoding='utf-8') as f:
                    authors = f.read().strip()
            
            # Extract category
            category_path = os.path.join(paper_dir, 'metadata', 'primary_category.txt')
            if os.path.exists(category_path):
                with open(category_path, 'r', encoding='utf-8') as f:
                    category = f.read().strip()
            # Fall back to category.txt if primary_category.txt doesn't exist
            elif os.path.exists(os.path.join(paper_dir, 'metadata', 'category.txt')):
                with open(os.path.join(paper_dir, 'metadata', 'category.txt'), 'r', encoding='utf-8') as f:
                    category = f.read().strip()
